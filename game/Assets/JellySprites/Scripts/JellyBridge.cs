			...
			if(jellySprite.m_Style == JellySprite.PhysicsStyle.Grid)				
			{
				...
				for(int x = 0; x < jellySprite.m_GridColumns; x++)	
				{
					...
					if(jellySprite.ReferencePoints[pointIndex].GameObject)
					{
						...
						if(jellySprite.ReferencePoints[pointIndex].GameObject.rigidbody)
						{
							jellySprite.ReferencePoints[pointIndex].GameObject.rigidbody.isKinematic = true;	
						}

						if(jellySprite.ReferencePoints[pointIndex].GameObject.rigidbody2D)	
						{
							jellySprite.ReferencePoints[pointIndex].GameObject.rigidbody2D.isKinematic = true;	
						}
					}
				}
			}
